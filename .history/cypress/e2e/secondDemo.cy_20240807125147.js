describe('Template Spec', () => {
  ...
  it('should test the select functionality', () => {
    ...
    cy.get('#superheros').select('ca').should('have.value', '[ca]');
    cy.get('#superheros').select('Captain America').should('have.value', '[ca]');

    //cy.get('#superheros').contains('[value="ca"]').scrollIntoView().should('be.visible').click();
