describe("Template Spec", () => {
  ...
  it("should test the frame functionality", () => {
    ...
    cy.frameLoaded('#firstFr'); // Ensure the iframe is loaded
    cy.iframe().find("input[placeholder='Enter name']").type("Benedicta Djangma");
    cy.iframe().find("input[placeholder='Enter email']").type("bene@gmail.com");
  });

  it("should test the toggle functionality", () => {
    cy.xpath("//a[normal").click();
    cy.xpath("//div[@class='card-content']//div[1]//div[1]//label[1]").should("be.checked");
  });
});
